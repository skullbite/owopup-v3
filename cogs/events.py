...
class Events(commands.Cog):
    ...
    async def on_command_error(self, ctx, err):
        ...
        if isinstance(err, errors.MissingRequiredArgument) or isinstance(
            ...
            await ctx.send(
                f"I'm missing permissions to run this command: `{' '.join(err.missing_perms)}`\nPlease make sure I have these before trying again."
            )
        elif isinstance(err, errors.MissingPermissions):
            print("uhhh")
            await ctx.send(
                f"You're missing permissions to run this command: `{' '.join(err.missing_perms)}`\nPlease make sure you have these before trying again."
            )
            ...
            if self.bot.abuse[f"{ctx.author.id}_{ctx.command.name}"] == 5:
                ...
                return await ctx.send(
                    "You've been blacklisted from commands for spamming them. You may use commands again in 15 minutes."
                )
            await ctx.send(
                f"Slow your roll. This command can't be used again for another {err.retry_after:.0f} seconds."
            )
            print("└[COOLDOWN]")
    ...
    async def on_ready(self):
        ...
        await self.bot.change_presence(
            activity=discord.Game(type=0, name="Enjoying V3 | owo announcement"),
    ...
    async def on_guild_join(self, guild):
        ...
        sketchy = (
            ...
            if guild.member_count < (guild.member_count - b)
            else "\n"
        )
        self.bot.webhook.send(
    ...
    async def post(self):
        if (
            not self.bot.is_ready() or not self.bot.user.id == 365255872181567489
        ):  # Beta Reasons
            return
        ...
        async with aiohttp.ClientSession() as send:
            await send.post(
                ...
                headers={"Authorization": self.bot.config.listtokens.dbl},
                data=payload,
            )
            await send.post(
                f"https://discord.bots.gg/api/v1/bots/{self.bot.user.id}/stats",
                headers={"Authorization": self.bot.config.listtokens.botsgg},
                data=payload2,
            )
            await send.post(
                f"https://botsfordiscord.com/api/bot/{self.bot.user.id}",
                headers={"authorization": self.bot.config.listtokens.bfd},
                data=payload,
            )
            await send.post(
                f"https://bots.ondiscord.xyz/bot-api/bots/{self.bot.user.id}/guilds",
                headers={"Authorization": self.bot.config.listtokens.bod},
                data=payload3,
            )
    ...
    async def on_command(self, ctx):
        try:
            print(
                f"[{ctx.guild.shard_id}][User: {ctx.author} ({ctx.author.id})][Command: {ctx.command.name}]"
            )
        except:
            print(
                f"[DM][User: {ctx.author} ({ctx.author.id})][Command: {ctx.command.name}]"
            )
