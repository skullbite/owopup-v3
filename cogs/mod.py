...
class Moderation(commands.Cog):
    ...
    async def ml(self, ctx, target: discord.User, case: str, reason, t_channel: discord.TextChannel=None):
        casetypes = {"unmute": 0x8534bf, "mute": 0xdb781a, "kick": 0xedd311, "ban": 0xed2311, "unban": 0x8534bf, "test": 0xffffff}
        if t_channel:
            channel = t_channel
        else:
        ...
        try:
            ...
            if case != "test":
                await ctx.send("⚠️ I couldn't send to the set modlog channel. Please consider changing the channel or making sure i can send to it.")
            return False
    ...
    async def settings(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            ...
            if not self.bot.get_channel(s["modlog"]) and modlog:
                modlog = "Possibly deleted/invisible channel. Please consider changing it."
