...
class Information(commands.Cog):
    ...
    async def help(self, ctx, *, cm: str = None):
        ...
        if not cm:
            ...
            mods = f"• {mods}"
            embed.title = (
                f"{paws} Hewwo {ctx.author.name}, I'm {self.bot.user.name}! {paws}"
            )
            ...
            ).replace(f"<@!{self.bot.user.id}>", f"@{self.bot.user.name}")
            ...
            try:
                subcmds = (
                    ...
                    + "\n".join(
                        [
                            f"│├{x.name} - {x.help}"
                            for x in self.bot.all_commands[cmd.name].commands
                        ]
                    )
                    + "```"
                )
    ...
    async def info(self, ctx):
        ...
        embed = discord.Embed(
            title="Here's some fun facts about me", color=self.bot.color
        )
